...
export function buildChatQueryParserUserPrompt(
  ...
  return [
    ...
    '- aggregate_pitching filters: year, year_from, year_to, game_date, pitcher_name, team, limit, sort_by (era|whip|strikeouts|wins|games|inningsPitched|hitsAllowed|walks|earnedRuns), min_innings_per_start (number: per-game minimum innings pitched), max_earned_runs_per_start (number: per-game maximum earned runs). 例: 「7回以上かつ自責点0の登板数ランキング」→ min_innings_per_start:7, max_earned_runs_per_start:0, sort_by:"games"',
    ...
    '- Set recent: true in search_batting whenever the user asks for recent individual game results using words like 最近, 直近, 最新, 最後, いつ, 今どんな, 調子. This bypasses the season-aggregate BIS stats and returns the most recent game-by-game batting lines instead. Example: "牧秀悟の最近の打撃成績" → {"intent":"search_batting","filters":{"player_name":"牧秀悟","year":2026,"recent":true}}.',
    '- Set recent: true in search_pitching whenever the user asks for the last/most recent appearance using words like 最後, 直近, 最新, 最近, いつ. Do NOT set year filter when the user asks for the last appearance without specifying a year — let the multi-year search find the most recent records across all seasons. Example: "藤浪晋太郎が最後に登板したのはいつ" → {"intent":"search_pitching","filters":{"pitcher_name":"藤浪晋太郎","recent":true}}.',
