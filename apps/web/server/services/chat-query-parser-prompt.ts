...
export function buildChatQueryParserUserPrompt(
  ...
  return [
    ...
    '- aggregate_batting filters: year, year_from, year_to, game_date, player_name, team, result_text_contains, limit, sort_by (hits|atBats|runsBattedIn|stolenBases|walks|strikeouts|battingAverage|games)',
    ...
    '- aggregate_batting/aggregate_pitching の sort_by: ランキング系の質問では必ず sort_by を設定すること。例: 防御率ランキング→sort_by:"era", WHIP→sort_by:"whip", 奪三振ランキング→sort_by:"strikeouts", 打率ランキング→sort_by:"battingAverage", 安打数ランキング→sort_by:"hits", 打点→sort_by:"runsBattedIn"。sort_byが不明な場合は省略可。',
