...
export const chatQueryParserSystemPrompt = [
  ...
  'The intent field must use the allowed contract values above. Repository intents select data access; off_topic is the explicit non-NPB boundary value. Follow-up classifications and response policies such as clarification, detail_request, correction_request, recheck_request, context_reference, explanation_request, or scope_clarification MUST NEVER be emitted as intent.',
  'off_topic means only that the message should not be handled as an NPB chat question. Missing context, a missing follow-up target, ambiguous entities, and missing history are never off_topic.',
  'Unresolved follow-up references are handled by the Planner response-policy layer before query parsing. Never encode clarification as a query intent.',
  ...
  'The exact-name rule applies to every person field: "田中将大" must remain "田中将大", "西川龍馬" must remain "西川龍馬", and "丸佳浩" must remain "丸佳浩". Never emit a prefix such as "田中将".',
  'CRITICAL: Never expand a surname-only input to a guessed full name. If the user wrote only "田中", the output name must remain exactly "田中" so Entity Resolution can report ambiguity. A name field must be a literal substring of the current user message or inherited conversation target.',
  ...
  'When the user reacts with dissatisfaction or asks for a re-answer after the previous assistant message (for example, "答えになってない", "違う", "再回答して"), preserve the previous repository query intent and filters. Application code classifies the follow-up metadata after this JSON is validated.',
  'For short or messy follow-ups, keep the current target from history and translate the surface form into one of the allowed repository query intents and filters. Never output follow-up metadata labels as intent.',
...
export function buildChatQueryParserUserPrompt(
  ...
  return [
    ...
    '',
    'Output JSON shape:',
    '{"intent":"search_events|search_games|search_batting|search_pitching|search_roster|player_affiliation|game_detail|aggregate_batting|aggregate_pitching|aggregate_events|aggregate_games|award_winners|off_topic","filters":{...}}',
    '',
    'Filter rules:',
    ...
    '- search_games filters: year, year_from, year_to, game_date, game_id, team, opponent, venue, competition, include_farm, limit',
    ...
    '- search_pitching filters: year, year_from, year_to, game_date, game_id, pitcher_name, pitcher_names, pitcher_player_id, pitcher_player_ids, team, level (first|farm), recent, sort_by (date|pitchCount), limit',
    ...
    '- game_detail filters: year, year_from, year_to, game_id, game_date, team, opponent, venue, competition, player_name, limit',
    '- aggregate_batting filters: year, year_from, year_to, game_date, player_name, player_id, team, result_text_contains, batting_order, position, group_by (player|year), limit, sort_by (hits|atBats|homeRuns|runsBattedIn|stolenBases|walks|strikeouts|battingAverage|ops|isoP|bbRate|games)',
    ...
    '- IsoPランキングは aggregate_batting + sort_by:"isoP"、四球率/BB%ランキングは aggregate_batting + sort_by:"bbRate" を使う。四球数のランキングへ置き換えないこと。',
    '- 年別成績を求められたら対象期間をyear_from/year_toで保持し、aggregate_battingではgroup_by:"year"を設定する。単一年へ縮めないこと。',
    ...
    '- 「二軍」「ファーム」「イースタン」「ウエスタン」は team フィルタに入れないこと（球団名ではない）。ファームでの登板数・成績を聞かれた場合、team フィルタを省略し pitcher_name、year、level:"farm"で search_pitching を使うこと。一軍を明示された場合はlevel:"first"を使う。',
    ...
    '- Distinguish one latest appearance from a recent-form aggregate. A singular phrase such as 「直近試合」「直近の試合」「最新登板」「最後に投げた試合」 means search_pitching with recent:true and limit:1. An explicit plural phrase such as 「直近5試合」「最近5試合」「ここ5登板」 uses that explicit limit. The elliptical phrase 「選手名の直近の内容」 does not name a singular 試合 or 登板, so treat it as a recent-form aggregate with limit:5. A vague 「最近の投球成績」 may also remain a recent multi-game summary.',
    ...
    '- For questions asking which single appearance had the most pitches, such as "今シーズン最も球数が多かった登板" or "球数が最も多かった登板", use search_pitching with sort_by:"pitchCount" and limit:1. For the longest single outing / most innings pitched appearance, such as "最も長く投げた登板" or "最長登板", use search_pitching with sort_by:"inningsPitched" and limit:1. Do not output sort_by:"pitches".',
    ...
    '- If the query asks for a starting lineup (スタメン) on a specific date/game, use search_roster with game_date, team, and starter:true. Example: "2026年5月10日の広島のスタメン" → {"intent":"search_roster","filters":{"game_date":"2026-05-10","team":"広島","starter":true}}.',
    ...
    '- 日本シリーズなど大会を指定した試合検索は competition を必ず保持する。例: "2025年の日本シリーズ" → search_games + year:2025 + competition:"日本シリーズ"。',
    '- 特定2球団の対戦カードは一方をteam、他方をopponentに入れる。球場も明示された場合はvenueを同時に保持する。例: "東京ドームのDeNA対巨人" → {"team":"DeNA","opponent":"巨人","venue":"東京ドーム"}。条件を一つでも落とさないこと。',
    '- venue、team、opponentは必ず分離し、teamへ球場名や「の」を含めないこと。"東京ドームのDeNA対巨人"のteamは厳密に"DeNA"、opponentは"巨人"、venueは"東京ドーム"である。',
    '- チームの球場別勝敗や対戦成績は aggregate_games を使い、teamに加えてvenueまたはopponentを保持する。打撃成績へ変換しないこと。',
    '- 「甲子園での阪神の成績」のように球場でのチーム成績を聞く質問は aggregate_games + team:"阪神" + venue:"甲子園" とする。チームの打撃成績ではない。',
    '- 打順・守備位置の起用回数で最多を聞かれた場合は aggregate_batting を使い、batting_order/position、sort_by:"games"、質問が一人を求める場合はlimit:1を保持する。',
