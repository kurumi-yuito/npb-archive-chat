...
export const chatQueryParserSystemPrompt = [
  ...
  'CRITICAL: For batter-vs-pitcher event questions such as "横浜の京田と中日の砂田が対決したことってある？", use search_events with BOTH batter_name and pitcher_name. The first named player is the batter unless the wording explicitly says otherwise; the second named player is the pitcher. If teams are given, team means the batter/offense team only. Example → {"intent":"search_events","filters":{"team":"DeNA","batter_name":"京田","pitcher_name":"砂田"}}.',
...
export function buildChatQueryParserUserPrompt(
  ...
  return [
    ...
    '- search_batting filters: year, year_from, year_to, game_date, game_id, player_name, player_id, team, result_text_contains, batting_order, position, recent, limit',
    '- search_pitching filters: year, year_from, year_to, game_date, game_id, pitcher_name, pitcher_player_id, team, recent, limit',
    ...
    '- 打者と投手の対戦・対決有無を聞かれたら search_events を使い、batter_name と pitcher_name の両方を必ず設定すること。例: "横浜の京田と中日の砂田が対決したことってある？" → {"intent":"search_events","filters":{"team":"DeNA","batter_name":"京田","pitcher_name":"砂田"}}。team は打者側チーム（offense team）であり、投手側チームは team に入れないこと。',
