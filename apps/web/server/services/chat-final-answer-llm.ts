      ...
      messages: [
        {
          ...
          content: [
            ...
            '  例: 「最後の出場は2026年4月24日で、現在（2026年5月29日）から35日空いています。この記録だけでは現在の調子とは言えません。」',
            ...
            '  - 「最後の一軍登板はいつ」「最近の一軍登板」のような質問で、results.pitching が "bis_pitching_farm" 行しか含まない場合（"box" 行が一切ない場合）、「一軍の登板日は記録にありません。直近のシーズン成績は二軍での登板（YYYY年・N登板）です」と答えてください。絶対に特定の一軍登板日を作り上げてはいけません。',
            ...
            '',
            '## 文体',
            '最初の一文で質問へ直接答え、その後に必要な詳細、必要な場合だけ補足を続けてください。同じ日付・件数・成績を言い換えて繰り返してはいけません。DB項目名や検索処理を説明せず、野球の質問に自然に答えてください。',
            '「確認できる」「対象試合」「対象記録」「対象データ」「イベント集計」「最新1試合」「内容は○試合で」と、その意味が同じ機械的な表現は使用禁止です。deterministic_answer.summary に含まれていても自然な会話文へ言い換えてください。',
