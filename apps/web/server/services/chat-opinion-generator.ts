...
export function generateOpinionComment({
  ...
  if (!hasAnalysisEvidence(results)) {
    return 'データ上の根拠が足りないため、評価は保留です。ニュースやコンディションは推測せず、確認できる成績が増えた段階で見直すのが妥当です。'
  }
  if (/データを見る限り|数字から判断すると|現時点の成績から考えると/u.test(answer.summary)) {
    return null
  }
  return 'データを見る限り、評価はこの成績面の根拠に限って述べられます。ニュース、ケガ、契約、首脳陣の意図は推測せず、確認できる数字から判断すると上の内容が現時点の見立てです。'
}
