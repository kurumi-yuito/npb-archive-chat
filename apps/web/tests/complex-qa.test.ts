 ...
 *
 * このテストは stub parser + 実DB（multi-year SQLite）を使って動作確認する。
 * LLMなしで動作する範囲のクエリを中心に、DB層・formatter・解決ロジックの網羅的テストを行う。
 */
...
function getService() {
  ...
  return createChatService(queryService, {
    parseStructuredQueryFromMessage: async (msg, ctx) => parseStructuredQueryFromMessageStub(msg),
  })
}
...
// カテゴリ8: イベント系クエリの検証
// ─────────────────────────────────────────────
describe('カテゴリ8: イベントクエリ（非標準event_type対応確認）', () => {
