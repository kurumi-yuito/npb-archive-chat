...
Serviceはstandalone質問のEntity Resolution直前に、Plannerの`targetEntity.players`と実行対象filterの人物名が一致することを境界不変条件として検査する。既存の安定化rewriteが短縮名やIDを再導入して人物名が不一致になった場合だけPlanner projectionへ戻し、IDと明示されていないteamを除去してから通常のResolverへ渡す。履歴から確定済みIDを継承するfollow-upには適用しない。この処理は人物の再分類やID推測を行わない。
