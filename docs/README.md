...
| 環境変数 / secret の設定先を確認する | [env-reference.md](./env-reference.md) |
...
| 本番投入時の確認項目を見る | [production-readiness.md](./production-readiness.md) |
...
本番インフラの確認作業は [production-readiness.md](./production-readiness.md) に集約する。D1 の本番データ同期手段 `sync:d1` は実装済みで、実行手順は [deploy.md](./deploy.md) にある。
